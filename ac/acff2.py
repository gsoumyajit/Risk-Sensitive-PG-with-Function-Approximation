...
fr.write("timestep\treward\treward1\trcost\n")
...
for n in range(N):
    ...
    Bi-=np.outer(np.dot(Bi,feat(state)),np.dot(feat(state),Bi))/(1+np.dot(np.dot(feat(state),Bi),feat(state)))
    ...
    if n%1000000==0:
        ...
        rcost=np.log(np.mean(np.exp(np.multiply(returns,k))))
        print(n//1000000,state,np.mean(returns),np.std(returns),rcost)
        fr.write(str(n)+'\t'+str(np.mean(returns))+'\t'+str(np.std(returns))+'\t'+str(rcost)+'\n')
        fr.flush()
